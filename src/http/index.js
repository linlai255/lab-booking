...
axios.interceptors.response.use(
  (response) => {
    if (response.status == 200) {
      ...
      return Promise.resolve(response)
    } else if (response.data.code && response.data.code >= 11000 && response.data.code <= 11007) {
      console.log('error')
...
export {apis, $http, $get, $getWithBody, $getWithPathVar, $getWithPathVarBody, $post, $put, $deleteByPathVar}
